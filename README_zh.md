...
- 支持同时满足 ES6 与 `ReadableStream` 基线的浏览器，版本矩阵按两者中的较大值取整：Chrome 65+、Edge 79+、Safari 10.1+、FireFox 65+、Opera 52+，以及其他等效的 Chromium 系浏览器
...
使用 global 构建时，需要把 `mpegts.global.js`、`transmuxing-worker.js` 和 `player-engine-worker.js` 一起部署到同一目录。
...
- 运行时基线要求同时满足 ES6 与 `ReadableStream`，版本矩阵按两者中的较大值取整：Chrome 65+、Edge 79+、Safari 10.1+、FireFox 65+、Opera 52+（[Can I Use: ES6][caniuse-es6]、[Can I Use: ReadableStream][caniuse-readablestream]）
...
- 兼容同一 ES6 + `ReadableStream` 基线的浏览器：Chrome 65+、Edge 79+、Safari 10.1+、FireFox 65+、Opera 52+
- 超低开销并且由你的浏览器硬件加速
