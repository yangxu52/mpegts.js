...
- ES6 と `ReadableStream` の両方の基線を満たすブラウザで実行可能で、バージョン行列はその大きい方に合わせます: Chrome 65+、Edge 79+、Safari 10.1+、FireFox 65+、Opera 52+、および同等の Chromium-based ブラウザ
...
global build を使う場合は、`mpegts.global.js`、`transmuxing-worker.js`、`player-engine-worker.js` を同じディレクトリに配置してください。
...
- ランタイム基線は ES6 と `ReadableStream` の両方を要求し、バージョン行列はその大きい方に合わせます: Chrome 65+、Edge 79+、Safari 10.1+、FireFox 65+、Opera 52+（[Can I Use: ES6][caniuse-es6]、[Can I Use: ReadableStream][caniuse-readablestream]）
...
- 同じ ES6 + `ReadableStream` 基線を満たすブラウザで実行可能: Chrome 65+、Edge 79+、Safari 10.1+、FireFox 65+、Opera 52+
