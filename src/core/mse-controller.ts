...
type ManagedMediaSource = MediaSource & {
  streaming?: boolean
...
type SourceBufferMap = Record<SegmentType, SourceBuffer | null>
type MimeTypeMap = Record<SegmentType, string | null>
type PendingSegmentMap = Record<SegmentType, (InitSegment | MediaSegment)[]>
type PendingRemoveRangeMap = Record<SegmentType, PendingRemoveRange[]>
type LastInitSegmentMap = Record<SegmentType, InitSegment | null>
...
class MSEController {
  private readonly TAG: string = 'MSEController'
  private _config: Config
  private _emitter: EventEmitter
  private _useManagedMediaSource: boolean
  private _mediaSource: ManagedMediaSource | null
  ...
  private _mimeTypes: MimeTypeMap
  private _sourceBuffers: SourceBufferMap
  private _lastInitSegments: LastInitSegmentMap
  private _pendingSegments: PendingSegmentMap
  private _pendingRemoveRanges: PendingRemoveRangeMap
  private e: EventMap | null
  ...
  initialize(mediaElementProxy: MediaElementProxy): void {
    ...
    const ManagedMediaSourceCtor = (self as typeof self & { ManagedMediaSource?: ManagedMediaSourceConstructor }).ManagedMediaSource
    let ms = (this._mediaSource = this._useManagedMediaSource ? new ManagedMediaSourceCtor!() : new self.MediaSource())
  ...
  shutdown(): void {
    if (this._mediaSource) {
      ...
      for (let type of Object.keys(this._sourceBuffers) as SegmentType[]) {
        ...
        if (sb) {
          if (ms.readyState !== 'closed') {
            ...
            try {
              ...
              Log.e(this.TAG, err.message ?? String(error))
            }
  ...
  getHandle(): MediaSourceHandle {
    if (!this._mediaSource || this._mediaSource.handle == null) {
      throw new IllegalStateException('MediaSource handle has not been initialized yet!')
    }
  ...
  appendInitSegment(initSegment: InitSegment, deferred = undefined): void {
  ...
  flush() {
    ...
    for (let type in this._sourceBuffers) {
      ...
      // Some Safari builds may reject later appendBuffer() after SourceBuffer.remove().
      // Re-append the last init segment to recover the internal parser state.
