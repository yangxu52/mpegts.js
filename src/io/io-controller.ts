...
import { LoaderStatus, LoaderErrors } from './loader'
import type { BaseLoader, DataSource, LoaderErrorInfo, LoaderErrorType, LoaderStatusType, SeekHandler, SeekRange } from './loader'
...
type IOControllerDataSource = DataSource & {
  url?: string
  filesize?: number
...
type LoaderInstance = BaseLoader & {
  currentSpeed?: number
}

type LoaderClass = {
  new (...args: any[]): LoaderInstance
  isSupported?: () => boolean
}

type OnDataArrivalCallback = ((chunks: ArrayBuffer, byteStart: number) => number) | null
type OnSeekedCallback = (() => void) | null
type OnErrorCallback = ((type: LoaderErrorType, info: LoaderErrorInfo) => void) | null
type OnCompleteCallback = ((extraData: unknown) => void) | null
type OnRedirectCallback = ((redirectedURL: string) => void) | null
type OnRecoveredEarlyEofCallback = (() => void) | null

// Manage IO Loaders
class IOController {
  private readonly TAG: string = 'IOController'
  ...
  private _loader: LoaderInstance | null
  private _loaderClass: LoaderClass | null
  private _seekHandler: SeekHandler | null
  private _dataSource: IOControllerDataSource
  ...
  private _onDataArrival: OnDataArrivalCallback
  private _onSeeked: OnSeekedCallback
  private _onError: OnErrorCallback
  private _onComplete: OnCompleteCallback
  private _onRedirect: OnRedirectCallback
  private _onRecoveredEarlyEof: OnRecoveredEarlyEofCallback

  constructor(dataSource: IOControllerDataSource, config: Config, extraData?: unknown) {
    this._config = config
  ...
  destroy(): void {
    ...
    this._loaderClass = null
    this._dataSource = {}
    this._stashBuffer = new ArrayBuffer(0)
  ...
  get status(): LoaderStatusType {
    return this._loader ? this._loader.status : LoaderStatus.kIdle
  }
  ...
  get onSeeked(): OnSeekedCallback {
    return this._onSeeked
  }

  set onSeeked(callback: OnSeekedCallback) {
  ...
  get onRedirect(): OnRedirectCallback {
    return this._onRedirect
  }

  set onRedirect(callback: OnRedirectCallback) {
    this._onRedirect = callback
  }

  get onRecoveredEarlyEof(): OnRecoveredEarlyEofCallback {
    return this._onRecoveredEarlyEof
  }

  set onRecoveredEarlyEof(callback: OnRecoveredEarlyEofCallback) {
  ...
  get currentSpeed(): number {
    ...
    if (this._loaderClass === RangeLoader) {
      ...
      return (this._loader as LoaderInstance & { currentSpeed: number }).currentSpeed
    }
    return this._speedSampler ? this._speedSampler.lastSecondKBps : 0
  }

  get loaderType(): string {
    return this._loader ? this._loader.type : 'undefined'
  }
  ...
  _selectSeekHandler(): void {
    ...
    if (config.seekType === 'range') {
      ...
      this._seekHandler = new config.customSeekHandler() as SeekHandler
    } else {
  ...
  _createLoader(): void {
    if (this._loaderClass == null || this._seekHandler == null) {
      throw new IllegalStateException('IOController: loader or seek handler has not been selected!')
    }
  ...
  _internalSeek(bytes: number, dropUnconsumed: boolean): void {
    ...
    let requestRange: SeekRange = { from: bytes, to: -1 }
  ...
  _dispatchChunks(chunks: ArrayBuffer, byteStart: number): number {
  ...
  _onLoaderChunkArrival(chunk: ArrayBuffer, byteStart: number, _receivedLength?: number): void {
    ...
    let KBps = this._speedSampler ? this._speedSampler.lastSecondKBps : 0
    if (KBps !== 0) {
  ...
  _onLoaderComplete(_from: number, _to: number): void {
  ...
  _onLoaderError(type: LoaderErrorType, data: LoaderErrorInfo): void {
