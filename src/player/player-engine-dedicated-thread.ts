...
import { createPlayerEngineWorker } from '../utils/worker-factory'
...
class PlayerEngineDedicatedThread implements PlayerEngine {
  ...
  public static isSupported(): boolean {
    ...
    if (self.MediaSource && 'canConstructInDedicatedWorker' in self.MediaSource && self.MediaSource['canConstructInDedicatedWorker'] === true) {
      return true
    }
    if (
  ...
  public load(): void {
    ...
    if (this._config.isLive && this._config.liveBufferLatencyChasing) {
      this._live_latency_chaser = new LiveLatencyChaser(this._config, this._media_element, this._onRequestDirectSeek.bind(this))
    }
