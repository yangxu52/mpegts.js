...
import AV1OBUParser, { type AV1Metadata } from './av1-parser'
...
type VideoMetadataDetails = ReturnType<typeof SPSParser.parseSPS> | ReturnType<typeof H265Parser.parseSPS> | AV1Metadata
type HEVCDecoderRecordDetails = Omit<ConstructorParameters<typeof HEVCDecoderConfigurationRecord>[3], 'configurationVersion'>

type TSDemuxProbeData =
  | {
      needMoreData: true
    }
  | {
...
type PESSliceContext = SectionSliceContext & {
...
class TSDemuxer extends BaseDemuxer {
  ...
  private video_metadata_: {
    ...
    details: VideoMetadataDetails | undefined
  } = {
    vps: undefined,
    sps: undefined,
    pps: undefined,
  ...
  public constructor(probe_data: Extract<TSDemuxProbeData, { match: true }>, config: any) {
    super()
  ...
  public static probe(buffer: ArrayBuffer): TSDemuxProbeData {
  ...
  private handleSectionSlice(buffer: ArrayBuffer, offset: number, length: number, misc: SectionSliceContext): void {
  ...
  private handlePESSlice(buffer: ArrayBuffer, offset: number, length: number, misc: PESSliceContext): void {
    let data = new Uint8Array(buffer, offset, length)
  ...
  private emitSectionSlices(slice_queue: SliceQueue, misc: SectionSliceContext): void {
  ...
  private emitPESSlices(slice_queue: SliceQueue, misc: PESSliceContext): void {
    let data = new Uint8Array(slice_queue.total_length)
    for (let i = 0, offset = 0; i < slice_queue.slices.length; i++) {
      let slice = slice_queue.slices[i]
      data.set(slice, offset)
  ...
  private clearSlices(slice_queue: SliceQueue, misc: SectionSliceContext | PESSliceContext): void {
    slice_queue.slices = []
    slice_queue.expected_length = -1
    slice_queue.total_length = 0
  }
  ...
  private parsePMT(data: Uint8Array): void {
    ...
    for (let i = info_start_index; i < info_start_index + info_bytes; ) {
      ...
      let already_has_audio =
        pmt.common_pids.adts_aac || pmt.common_pids.loas_aac || pmt.common_pids.ac3 || pmt.common_pids.eac3 || pmt.common_pids.opus || pmt.common_pids.mp3
  ...
  private parseAV1Payload(data: Uint8Array, pts: number, dts: number, file_position: number, random_access_indicator: number) {
    ...
    let details: AV1Metadata | null = this.video_metadata_.details as AV1Metadata | null
    while ((payload = av1_in_ts_parser.readNextOBUPayload()) != null) {
      details = AV1OBUParser.parseOBUs(payload, details)
  ...
  private detectVideoMetadataChange(new_sps: H264NaluAVC1 | H265NaluHVC1, new_details: VideoMetadataDetails): boolean {
    if (new_details.codec_mimetype !== this.video_metadata_.details.codec_mimetype) {
      Log.v(this.TAG, `Video: Codec mimeType changed from ` + `${this.video_metadata_.details.codec_mimetype} to ${new_details.codec_mimetype}`)
      return true
    }

    if (
      new_details.codec_size.width !== this.video_metadata_.details.codec_size.width ||
      ...
      Log.v(this.TAG, `Video: Coded Resolution changed from ` + `${old_size.width}x${old_size.height} to ${new_size.width}x${new_size.height}`)
      return true
    }
    ...
    if (new_details.present_size.width !== this.video_metadata_.details.present_size.width) {
      Log.v(
        this.TAG,
        `Video: Present resolution width changed from ` + `${this.video_metadata_.details.present_size.width} to ${new_details.present_size.width}`
      )
      return true
    }
  ...
  private isHEVCDecoderRecordDetails(details: VideoMetadataDetails): details is VideoMetadataDetails & HEVCDecoderRecordDetails {
    return 'general_profile_space' in details && 'num_temporal_layers' in details && 'parallelismType' in details
  }

  private createHEVCDecoderRecordDetails(details: HEVCDecoderRecordDetails): ConstructorParameters<typeof HEVCDecoderConfigurationRecord>[3] {
    return {
  ...
  private dispatchVideoInitSegment() {
    ...
    if (this.video_metadata_.av1c) {
      ...
      if (!this.isHEVCDecoderRecordDetails(details)) {
        throw new IllegalStateException('HEVC metadata is incomplete')
      }
      ...
      let hvcc = new HEVCDecoderConfigurationRecord(vps_without_header, sps_without_header, pps_without_header, this.createHEVCDecoderRecordDetails(details))
  ...
  private detectAudioMetadataChange(sample: AudioData): boolean {
    ...
    if (sample.codec === 'aac' && this.audio_metadata_.codec === 'aac') {
      ...
      if (frame.audio_object_type !== this.audio_metadata_.audio_object_type) {
        Log.v(this.TAG, `AAC: AudioObjectType changed from ` + `${this.audio_metadata_.audio_object_type} to ${frame.audio_object_type}`)
        return true
      }

      if (frame.sampling_freq_index !== this.audio_metadata_.sampling_freq_index) {
        Log.v(this.TAG, `AAC: SamplingFrequencyIndex changed from ` + `${this.audio_metadata_.sampling_freq_index} to ${frame.sampling_freq_index}`)
        return true
      }

      if (frame.channel_config !== this.audio_metadata_.channel_config) {
        Log.v(this.TAG, `AAC: Channel configuration changed from ` + `${this.audio_metadata_.channel_config} to ${frame.channel_config}`)
        return true
      }
      ...
      if (frame.sampling_frequency !== this.audio_metadata_.sampling_frequency) {
        Log.v(this.TAG, `AC3: Sampling Frequency changed from ` + `${this.audio_metadata_.sampling_frequency} to ${frame.sampling_frequency}`)
        return true
      }
      ...
      if (frame.bit_stream_identification !== this.audio_metadata_.bit_stream_identification) {
        Log.v(
          this.TAG,
          `AC3: Bit Stream Identification changed from ` + `${this.audio_metadata_.bit_stream_identification} to ${frame.bit_stream_identification}`
        )
        return true
      }

      if (frame.bit_stream_mode !== this.audio_metadata_.bit_stream_mode) {
        Log.v(this.TAG, `AC3: BitStream Mode changed from ` + `${this.audio_metadata_.bit_stream_mode} to ${frame.bit_stream_mode}`)
        return true
      }
      ...
      if (data.sample_rate !== this.audio_metadata_.sample_rate) {
        Log.v(this.TAG, `Opus: SamplingFrequencyIndex changed from ` + `${this.audio_metadata_.sample_rate} to ${data.sample_rate}`)
        return true
      }

      if (data.channel_count !== this.audio_metadata_.channel_count) {
        Log.v(this.TAG, `Opus: Channel count changed from ` + `${this.audio_metadata_.channel_count} to ${data.channel_count}`)
        return true
      }
      ...
      if (data.sample_rate !== this.audio_metadata_.sample_rate) {
        Log.v(this.TAG, `MP3: SamplingFrequencyIndex changed from ` + `${this.audio_metadata_.sample_rate} to ${data.sample_rate}`)
        return true
      }

      if (data.channel_count !== this.audio_metadata_.channel_count) {
        Log.v(this.TAG, `MP3: Channel count changed from ` + `${this.audio_metadata_.channel_count} to ${data.channel_count}`)
        return true
      }
    }
