...
import buffersAreEqual from '../utils/typedarray-equality'
import AV1OBUParser, { type AV1Metadata } from './av1-parser'
import ExpGolomb from './exp-golomb.js'
import { parseSEI, type SEIData } from './sei'
...
type FLVOnMetaData = {
  ...
  duration?: number
  framerate?: number
  keyframes?: FLVKeyframesIndex | null
} & Record<string, any>

type ScriptData = Record<string, any> & {
  onMetaData?: FLVOnMetaData | null
}
...
type VideoTrack = {
  ...
  id: number
  sequenceNumber: number
  samples: VideoTrackSample[]
  length: number
}

type AACAudioSpecificConfig = {
  config: Uint8Array
  samplingRate: number
  channelCount: number
  codec: string
...
type OnMetaDataArrivedCallback = ((metadata: Record<string, any>) => void) | null
type OnScriptDataArrivedCallback = ((data: Record<string, any>) => void) | null
type OnTrackMetadataCallback = ((type: 'audio' | 'video', metadata: AudioTrackMetadata | VideoTrackMetadata) => void) | null
type OnDataAvailableCallback = ((audioTrack: AudioTrack, videoTrack: VideoTrack) => void) | null
type OnSeiArrivedCallback = ((sei_data: SEIData) => void) | null
...
function createAudioTrackMetadata(id: number, timescale: number, duration: number): AudioTrackMetadata {
  return {
    type: 'audio',
    id,
    timescale,
...
function createVideoTrackMetadata(id: number, timescale: number, duration: number): VideoTrackMetadata {
  return {
    type: 'video',
    id,
    timescale,
...
function isAACAudioSpecificConfigData(data: AACAudioData): data is Extract<AACAudioData, { packetType: 0 }> {
  return data.packetType === 0
}
...
class FLVDemuxer {
  ...
  private _onMediaInfo: OnMediaInfoCallback
  private _onMetaDataArrived: OnMetaDataArrivedCallback
  private _onScriptDataArrived: OnScriptDataArrivedCallback
  private _onTrackMetadata: OnTrackMetadataCallback
  private _onDataAvailable: OnDataAvailableCallback
  private _onSeiArrived: OnSeiArrivedCallback
  ...
  private _metadata: ScriptData | null
  private _audioMetadata: AudioTrackMetadata | null
  private _videoMetadata: VideoTrackMetadata | null
  ...
  private _referenceFrameRate: FrameRate

  private _flvSoundRateTable: number[]
  private _mpegSamplingRates: number[]
  private _mpegAudioV10SampleRateTable: number[]
  private _mpegAudioV20SampleRateTable: number[]
  private _mpegAudioV25SampleRateTable: number[]
  private _mpegAudioL1BitRateTable: number[]
  private _mpegAudioL2BitRateTable: number[]
  private _mpegAudioL3BitRateTable: number[]

  private _videoTrack: VideoTrack
  private _audioTrack: AudioTrack
  private _littleEndian: boolean

  constructor(probeData: Extract<FLVProbeData, { match: true }>, config: any) {
    this.TAG = 'FLVDemuxer'
  ...
  static probe(buffer: ArrayBuffer): FLVProbeData {
    ...
    let mismatch: Extract<FLVProbeData, { match: false }> = { match: false }
  ...
  bindDataSource(loader: LoaderLike) {
    loader.onDataArrival = this.parseChunks.bind(this)
    return this
  }

  // prototype: function(type: string, metadata: any): void
  get onTrackMetadata(): OnTrackMetadataCallback {
    return this._onTrackMetadata
  }

  set onTrackMetadata(callback: OnTrackMetadataCallback) {
    this._onTrackMetadata = callback
  }

  // prototype: function(mediaInfo: MediaInfo): void
  get onMediaInfo(): OnMediaInfoCallback {
    return this._onMediaInfo
  }

  set onMediaInfo(callback: OnMediaInfoCallback) {
    this._onMediaInfo = callback
  }

  get onMetaDataArrived(): OnMetaDataArrivedCallback {
    return this._onMetaDataArrived
  }

  set onMetaDataArrived(callback: OnMetaDataArrivedCallback) {
    this._onMetaDataArrived = callback
  }

  get onScriptDataArrived(): OnScriptDataArrivedCallback {
    return this._onScriptDataArrived
  }

  set onScriptDataArrived(callback: OnScriptDataArrivedCallback) {
    this._onScriptDataArrived = callback
  }

  get onSeiArrived(): OnSeiArrivedCallback {
    return this._onSeiArrived
  }

  set onSeiArrived(callback: OnSeiArrivedCallback) {
  ...
  get onDataAvailable(): OnDataAvailableCallback {
    return this._onDataAvailable
  }

  set onDataAvailable(callback: OnDataAvailableCallback) {
  ...
  _parseAudioData(arrayBuffer, dataOffset, dataSize, tagTimestamp) {
    ...
    if (!meta) {
      ...
      // initial metadata
      meta = this._audioMetadata = createAudioTrackMetadata(track.id, this._timescale, this._duration)
    ...
    if (soundFormat === 10) {
      ...
      if (isAACAudioSpecificConfigData(aacData)) {
  ...
  _parseAACAudioData(arrayBuffer, dataOffset, dataSize): AACAudioData | undefined {
    if (dataSize <= 1) {
      Log.w(this.TAG, 'Flv: Invalid AAC packet, missing AACPacketType or/and Data!')
      return
    }

    let result: Partial<AACAudioData> = {}
    ...
    return result as AACAudioData
  }

  _parseAACAudioSpecificConfig(arrayBuffer, dataOffset, dataSize): AACAudioSpecificConfig | undefined {
    let array = new Uint8Array(arrayBuffer, dataOffset, dataSize)
  ...
  _parseAVCDecoderConfigurationRecord(arrayBuffer, dataOffset, dataSize) {
    ...
    if (!meta) {
      ...
      meta = this._videoMetadata = createVideoTrackMetadata(track.id, this._timescale, this._duration)
    } else {
  ...
  _parseAVCVideoData(arrayBuffer, dataOffset, dataSize, tagTimestamp, tagPosition, frameType, cts) {
    ...
    if (units.length) {
      let track = this._videoTrack
      let avcSample: VideoTrackSample = {
        units: units,
        length: length,
        isKeyframe: keyframe,
        dts: dts,
  ...
  _parseHEVCVideoData(arrayBuffer, dataOffset, dataSize, tagTimestamp, tagPosition, frameType, cts) {
    ...
    if (units.length) {
      let track = this._videoTrack
      let hevcSample: VideoTrackSample = {
        units: units,
        length: length,
        isKeyframe: keyframe,
        dts: dts,
  ...
  _parseAV1VideoData(arrayBuffer, dataOffset, dataSize, tagTimestamp, tagPosition, frameType, cts) {
    ...
    if (units.length) {
      let track = this._videoTrack
      let av1Sample: VideoTrackSample = {
        units: units,
        length: length,
        isKeyframe: keyframe,
        dts: dts,
